...
| 5 | **Microcoelus** | Microcoelus → Microsaurops | junior objective per Olshevsky (unnecessary replacement; see notes) | **Re-scoped to full Cat-III intake** (was verify-only). Modern consensus treats Microcoelus Lydekker 1893 as an indeterminate saltasaur historically conflated with Neuquensaurus, not specifically Saltasaurus. Split into its own `genera/M/Microcoelus.yml`, anchored to Lydekker 1893 (holotype MLP-Ly 23) + a modern titanosaur revision paper. Remove the Microcoelus synonym block from `Saltasaurus.yml` in the same commit. Microsaurops Kuhn 1963 is an unnecessary replacement based on Lull 1910's typo "Microsaurus" — mention in description prose but do not give synonym-block weight. |
